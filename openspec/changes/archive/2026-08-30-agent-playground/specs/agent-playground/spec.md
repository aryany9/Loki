...
### Requirement: Provider Selection Controls (deferred)

Speech provider (ASR/TTS) selection controls are OUT OF SCOPE for this change and will be added with the Voice Assistant workflow (Change 4). No provider selectors are introduced here.

#### Scenario: (no scenario — deferred)
