...
The prompt assembly pipeline SHALL maintain distinct sections for **Tool Protocol** (syntax: how tools are invoked via JSON) and **Turn Protocol** (semantics: how turn transitions occur). In `VOICE` mode, Turn Protocol SHALL instruct that `ask_user` MUST be invoked if the task requires another user utterance before continuing. In `CHAT` mode, Turn Protocol SHALL instruct that questions be asked directly in natural conversational text.
