...
**Reason**: `llama.cpp` and GGUF support are removed to make Loki a clean, single-runtime LiteRT-LM native application.
**Migration**: Use `LiteRtLlmEngine` with `.litertlm` models.

### Requirement: `LlmEngine` supports grammar-constrained output
**Reason**: GBNF grammar constraints were specific to llama.cpp. LiteRT-LM structured output follows its own schema/tooling capabilities.
**Migration**: Tool calling and structured generation use LiteRT-LM native tool/message definitions.
