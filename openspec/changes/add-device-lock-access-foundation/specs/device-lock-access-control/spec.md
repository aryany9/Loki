...
The system SHALL provide an `AndroidDeviceLockStateProvider` implementation in `core:assistant` that inspects the Android `KeyguardManager.isKeyguardLocked` property using the application context.
...
The system SHALL provide an `AllowAllToolAccessPolicy` implementing `ToolAccessPolicy` that unconditionally returns `AccessDecision.Allow` for all tools and lock states, serving as the default implementation during foundation rollout.

#### Scenario: Allow all policy evaluates any tool
- **WHEN** `AllowAllToolAccessPolicy.evaluate(tool, lockState)` is called with any tool and any `DeviceLockState`
