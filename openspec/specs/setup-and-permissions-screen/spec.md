...
- **WHEN** the app is launched for the first time (no `first_run_complete` flag in DataStore)
- **THEN** `SetupScreen` is shown instead of `ChatScreen`

#### Scenario: Returning user skips setup
- **WHEN** the app is launched and `first_run_complete` is `true` in DataStore
- **THEN** `ChatScreen` is shown directly without displaying `SetupScreen`

#### Scenario: Setup completes after RECORD_AUDIO is granted
- **WHEN** the user grants `RECORD_AUDIO` on the setup screen
