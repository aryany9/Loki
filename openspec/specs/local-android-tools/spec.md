...
When a requested contact name matches multiple contacts, the model SHALL present the matching contacts BY NAME ONLY (without speaking or displaying phone numbers; numbers are reserved for the actual `call_contact` invocation) and ask the user which one to call before invoking `call_contact` for a single selection. If more than 3 contacts match, the model SHALL group or summarize the list to keep the spoken response short. A unique single match SHALL keep the existing direct-confirm flow.
